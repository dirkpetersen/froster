...
There were 2 Motivations to create `froster`: First, ChatGPT4 had just been released and almost 50% of Froster's code was generated by ChatGPT. The logo was autogenerated by https://www.brandcrowd.com/. And second, we are working with researchers to improve their archiving efforts. Here we want to learn about user workflows and what metadata needs to be captured.   
...
Some advanced configuration settings are not offered through a user inferface. All configuration settings are under `~/.config/froster`. for example `~/.config/froster/general` has one file easy per setting  
