...
[![Release](https://img.shields.io/github/v/release/dirkpetersen/froster)](https://github.com/dirkpetersen/froster/releases/latest)
...
The only OS prerequisite is the `fuse3` package, and only if you want to use
`froster mount`. Slurm is auto-detected when present.

To check that everything works, run `froster --info` or `froster --version`.
...
To update Froster, download the latest release binary and replace the old
one (same commands as in the [Installation section](#installation)).
The `froster update` self-updater is currently deferred (see
...
Froster v0.23 is a ground-up rewrite: it now ships as a single static Go
binary with the file system crawler, the transfer engine (rclone), and the
AWS integration built in. A few features of earlier releases (v0.22 and
before) have not been carried over yet and will return in future versions:

- **`froster config`** — the interactive configuration wizard, including
  `--import`/`--export`. Configuration is currently done by editing
  `~/.config/froster/config.ini` directly (see
  [Configuration](#configuration)). Existing configurations keep working
  unchanged.
- **`froster update`** — the self-updater; download the latest release
  binary instead.
- **`froster test`** — the self-test subcommand.
- **The interactive NIH grant search table** (`froster archive --nih`);
  passing a known grant ID with `--nih-ref <id>` works as before.
- **Restore to a cloud machine** (`froster restore --aws`,
  `--instance-type`, `--monitor`) and the related email notifications. The
  flags are still accepted and print a clear message.

Archives created with any earlier froster version restore unchanged — the
archive format, configuration files, and archive database are fully
...
Froster reads its configuration from `~/.config/froster/config.ini` plus the
standard AWS files (`~/.aws/credentials`, `~/.aws/config`). Existing
configurations from earlier froster versions work unchanged.

The interactive `froster config` wizard is currently deferred (see
[Deferred features](#deferred-features)), so configuration is done by
creating the files directly — a minimal setup:
...
`~/.aws/config` (add the nested `s3 =` block only for non-AWS endpoints
such as Ceph or Minio):
...
Verify your setup with `froster credentials`. For what every section and key
means (multiple profiles, shared team configuration, NIH, Slurm), check the
...
1. First, we may need to crawl the file system, which sometimes has billions of files, to find data that is actually worth archiving. For this, Froster has a built-in multi-threaded parallel file system crawler (output-compatible with the well-known [pwalk](https://github.com/fizwit/filesystem-reporting-tools)) that creates a large CSV file of all discovered file metadata.

1. Our focus is on archiving folders rather than individual files, as data in the same folder typically belongs together. To this end, we analyze the crawler's CSV output and generate a new list (CSV) of the largest folders sorted by size. (We call this a `Hotspots` file)

1. We pass this new list (CSV) to an interactive tool, which displays a table of folders alongside their total size in GiB, their average file sizes in MiB (MiBAvg), as well as their age in days since last accessed (AccD) and modified (ModD). Users can scroll down and right using the mouse and hitting enter will trigger the archiving of the selected folder.
...
1. We pass the folder to the built-in transfer engine (powered by [Rclone](https://rclone.org), embedded in froster) to execute a copy job.
...
> **Note**: the interactive `froster config` wizard is currently deferred
> (see [Deferred features](#deferred-features)); the sections below document
> every configuration setting so you can maintain
> `~/.config/froster/config.ini` directly — see
> [Configuration](#configuration) for a minimal starting point.

You should use the default DEEP_ARCHIVE for `AWS S3 storage class` as this is currently the most cost-effective storage solution. It takes only 48-72 hours to retrieve your data with the least expensive retrieval option ('Bulk'). See [AWS Storage Class guide](https://docs.aws.amazon.com/AmazonS3/latest/userguide/storage-class-intro.html). However, you can choose other [AWS S3 storage classes](https://rclone.org/s3/#s3-storage-class) supported by the rclone copy tool. 
...
> **Note**: the interactive grant search table is currently deferred (see
> [Deferred features](#deferred-features)); linking a known grant with
> `froster archive --nih-ref <grant-id>` works.

To invoke the search interface configure it as default in the config.ini `[NIH]` section or use the --nih option with `archive` subcommand
...
`froster index` crawls the file system and creates a large csv file with all file system information but this file is immediately deleted after running the index and all detailed file information is lost. You can save that original file (warning: it can be huge) using the `--pwalk-copy` option and then analyze that data later, e.g. with `visidata` (command: vd, hit q to quit) or [DuckDB](https://duckdb.org)
...
git clone https://github.com/dirkpetersen/froster.git
cd froster/go
go build ./cmd/froster     # → ./froster (the pinned Go toolchain downloads automatically)
go test ./...              # unit tests + Docker-guarded integration tests
```

Any change you make is reflected on the next `go build` — there is nothing
to install. Developer documentation (test matrix, package map,
compatibility artifacts, coding rules) lives in
...
Froster releases are automatically handled by GitHub Actions
([release workflow](.github/workflows/release-go.yml)): pushing a version
tag builds the binaries for all supported platforms, generates sha256
checksums, and publishes a GitHub release with the binaries attached.

The version follows this convention: `v<Major>.<Minor>.<Subminor>`
...
Then check the [release workflow run](../../actions/workflows/release-go.yml)
and the [releases page](../../releases).
...
To restore a specific folder, simply use the command followed by the desired path, for example: `s3-restore.sh /my/shared/folder`. It's worth noting that system administrators might hesitate to endorse complex tools they aren't familiar with. Fortunately, `s3-restore.sh` is a straightforward bash shell script, easily customizable to suit specific needs. 
