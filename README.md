...
There were 2 Motivations to create `froster`: First, ChatGPT4 had just been released and almost 50% of Froster's code was generated by ChatGPT. The logo was autogenerated by https://www.brandcrowd.com/. And second, we are working with researchers to improve their archiving efforts. Here we want to learn about user workflows and what metadata needs to be captured. As froster has a tiny codebase it will be very easy to adopt. Metadata could be captured through an addional web-interface but since a web system after requires IT assistance and an extra authentication layer it may be easier to capture such information through a [Textual TUI interface](https://github.com/mitosch/textual-select). The experience gained with `Froster` will likely inform the creation or purchasing of other, more comprehensive tools.
