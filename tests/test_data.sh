...
generate_test_data() {
    ...
    # Only set mtime with touch -h as atime can't be reliably set on symlinks
    touch -h -m -d '100 days ago' "$base_dir/link_to_subdir1"
    ln -s ".." "$base_dir/link_to_parent"
    touch -h -m -d '100 days ago' "$base_dir/link_to_parent"
    ...
    if [ -n "$target_medium" ]; then
        ...
        touch -h -m -d '100 days ago' "$base_dir/link_to_medium_file"
    fi
    if [ -n "$target_small" ]; then
        ln -s "$(basename "$target_small")" "$base_dir/link_to_small_file"
        touch -h -m -d '100 days ago' "$base_dir/link_to_small_file"
    fi
    if [ -n "$target_script" ]; then
        ln -s "$(basename "$target_script")" "$base_dir/link_to_script_file"
        touch -h -m -d '100 days ago' "$base_dir/link_to_script_file"
    fi
